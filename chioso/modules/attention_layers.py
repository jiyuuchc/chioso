...
def dot_product_attention(
    ...
    if capture_attention_weights:
        ...
        attn_weights = nn_layers.IdentityLayer(
            name='attn_weights')(attn_weights)

    # Apply attention dropout.
    if not deterministic and dropout_rate > 0.:
        if dropout_rng is None:
            raise ValueError(
                'Did not provide `rng` to dot_product_attention().')
...
def axial_dot_product_attention(
    ...
    # all dims but bs, heads, and channel.
...
class RelativeAttentionBias(nn.Module):
    ...
    def __call__(self) -> jnp.ndarray:
        ...
        for i, l in enumerate(self.nd_shape):
            ...
            if l > 1:
                new_bias = self.relative_attn_bias(
                    l, self.num_heads, f'bias_{i}')
                repeat = length // (tile * l)
                if repeat > 1:
                    new_bias = new_bias[:, :, jnp.newaxis, :, jnp.newaxis]
                    new_bias = jnp.tile(
                        new_bias, [1, tile, repeat, tile, repeat])
                    new_bias = jnp.reshape(
                        new_bias, [self.num_heads, length, length])
                elif tile > 1:
